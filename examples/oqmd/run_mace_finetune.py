#!/usr/bin/env python3
"""MACE fine-tuning on OQMD (formation energy per atom, periodic crystals).

Fine-tunes a MACE foundation model (default **MACE-MP-0**, materials/PBE) on the
OQMD train split via ``mace_run_train``, evaluates on the test split, and writes
``benchmark_results/mace_finetuned_summary.json``.

OQMD targets formation energy per atom (eV/atom).  MACE learns an *extensive*
...
``oqmd_getData.py`` now persists the lattice — rebuild ``dataset/oqmd.pickle``
if it predates that change.

Usage
-----
    python examples/oqmd/run_mace_finetune.py --device cuda
    python examples/oqmd/run_mace_finetune.py --epochs 50 --models mace_mp0_medium
"""
...
FINETUNE_MODEL_DIR = REPO_ROOT / "pretrained_model_ensemble" / "mace_finetuned" / "oqmd"
...
def main():
    ...
    for model_id in args.models:
        ...
        print(f"\n{'=' * 60}\nFine-tuning {label} on OQMD\n{'=' * 60}")
        work_dir = Path(tempfile.mkdtemp(prefix=f"mace_ft_{model_id}_oqmd_"))
