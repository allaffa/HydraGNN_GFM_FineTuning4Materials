#!/usr/bin/env python3
"""UMA fine-tuning on ABC3 (formation energy per atom, periodic crystals).

ABC3 (ABX3 perovskites) targets the DFT **formation energy per atom** (eV/atom).
Foundation MLIPs predict an *extensive* energy, so we fine-tune UMA (uma-s-1p2,
``omat`` task) to regress the total formation energy (= per-atom × n_atoms) and
report the per-atom MAE on the held-out test split.

There are no reference forces in ABC3, so training is energy-only.  Because the
...
Data object.  ``abc3_getData_API.py`` now persists the lattice; rebuild the
``dataset/abc3.pickle`` dataset if it predates that change.

Usage
-----
    python examples/abc3/run_uma_finetune.py --device cuda
    python examples/abc3/run_uma_finetune.py --epochs 30 --freeze-backbone
"""
...
def evaluate_split(model, calc, split, verbose=True) -> dict:
    ...
    for i, data in enumerate(dataset):
        atoms = pyg_data_to_ase_atoms(data, periodic=True)
        atoms.calc = calc
        e_pred_total = float(atoms.get_potential_energy())
        e_pred_per_atom = e_pred_total / len(atoms)
...
def main():
    ...
    print("Loading ABC3 train split …")
